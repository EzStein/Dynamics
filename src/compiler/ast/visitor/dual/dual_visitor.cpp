...
void dual_visitor::visit(binary_addition_operator_node* node1) {
  binary_addition_comparator_visitor visit(node1, callBackVisitor);
  node2->accept(&visit);
}
...
void dual_visitor::visit(unary_minus_operator_node* node) {
...
void dual_visitor::visit(variable_leaf_node* node) {
  variable_dual_visitor visit(node1, callBackVisitor);
  node2->accept(&visit);
}

void dual_visitor::visit(exponentiation_operator_node* node) {
  exponentiation_dual_visitor visit(node1, callBackVisitor);
  node2->accept(&visit);
}

void dual_visitor::visit(division_operator_node* node) {
